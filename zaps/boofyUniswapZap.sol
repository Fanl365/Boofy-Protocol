...
interface IBoofyVaultV6 is IERC20 {
    function deposit(uint256 amount) external;
    function withdraw(uint256 shares) external;
    function want() external pure returns (address);
}

contract BoofyUniV2Zap {
    using LowGasSafeMath for uint256;
    using SafeERC20 for IERC20;
    using SafeERC20 for IBoofyVaultV6;
    ...
    function beefInETH (address boofyVault, uint256 tokenAmountOutMin) external payable {
        require(msg.value >= minimumAmount, 'Boofy: Insignificant input amount');

        IWETH(WETH).deposit{value: msg.value}();

        _swapAndStake(boofyVault, tokenAmountOutMin, WETH);
    }

    function beefIn (address boofyVault, uint256 tokenAmountOutMin, address tokenIn, uint256 tokenInAmount) external {
        ...
        _swapAndStake(boofyVault, tokenAmountOutMin, tokenIn);
    }

    function beefOut (address boofyVault, uint256 withdrawAmount) external {
        (IBoofyVaultV6 vault, IUniswapV2Pair pair) = _getVaultPair(boofyVault);

        IERC20(boofyVault).safeTransferFrom(msg.sender, address(this), withdrawAmount);
    ...
    function beefOutAndSwap(address boofyVault, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) external {
        (IBoofyVaultV6 vault, IUniswapV2Pair pair) = _getVaultPair(boofyVault);
        address token0 = pair.token0();
        address token1 = pair.token1();
        require(token0 == desiredToken || token1 == desiredToken, 'Boofy: desired token not present in liqudity pair');
    ...
    function _getVaultPair (address boofyVault) private view returns (IBoofyVaultV6 vault, IUniswapV2Pair pair) {
        vault = IBoofyVaultV6(boofyVault);
        pair = IUniswapV2Pair(vault.want());
        require(pair.factory() == router.factory(), 'Boofy: Incompatible liquidity pair factory');
    }

    function _swapAndStake(address boofyVault, uint256 tokenAmountOutMin, address tokenIn) private {
        (IBoofyVaultV6 vault, IUniswapV2Pair pair) = _getVaultPair(boofyVault);

        (uint256 reserveA, uint256 reserveB,) = pair.getReserves();
        require(reserveA > minimumAmount && reserveB > minimumAmount, 'Boofy: Liquidity pair reserves too low');

        bool isInputA = pair.token0() == tokenIn;
        require(isInputA || pair.token1() == tokenIn, 'Boofy: Input token not present in liqudity pair');
    ...
    function estimateSwap(address boofyVault, address tokenIn, uint256 fullInvestmentIn) public view returns(uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        checkWETH();
        (, IUniswapV2Pair pair) = _getVaultPair(boofyVault);
    ...
    function checkWETH() public view returns (bool isValid) {
        ...
        require(isValid, 'Boofy: WETH address not matching Router.WETH()');
    }
